...
## (simply copy this setup section (without 'use vars ...') to your config file)
		...
		'de' => 
			{
				...
				names => 'Dateien/Ordner', lastmodified => 'Datum', size => 'Groesse', mimetype => ' MIME Typ',
...
sub getfancyfilename {
	...
	$ret = (!-d $fn && -r $fn) || -x $fn  ? $cgi->a({href=>$full,title=>$s,style=>'padding:1px'},$cgi->escapeHTML($fntext)) : $cgi->escapeHTML($fntext);
	...
	$full.=($q =~ /\?/)?';action=props':'?action=props';
...
sub readDirRecursive {
	...
	if ($depth!=0 &&  -d $nfn ) {
		opendir(DIR, $nfn);
		foreach my $f ( sort cmp_files grep { !/^(\.|\.\.)$/ } readdir(DIR) ) {
